...
// イベントハンドラについて
// 特定のイベントが発生した際に実行される処理と、その登録方法のことで、イベントリスナーと異なり、1つのイベントに複数の設定のすることができない。まだまだ主流なので、覚えた方が良い
// 方法は2つあり、1つは要素のプロパティに指定する方法と、HTMLファイルで要素の属性に直接記述する方法がある。
// イベントハンドラ名は基本的に先頭にonをつけたものになる。
...
/*<button id="test" onclick="elementclick();">aiueo</button> *****/
              // (イベントハンドラ名)=(関数の呼び出し)
